...
class Attention(nn.Module):
    ...
    def relu_linear_att(self, q, k, v, C):
        B, h, N, D = q.shape
        # [1, 32, 96, 496]
        ...
        # linear matmul
        trans_k = k.transpose(-1, -2)   # [1, 32, 32, 496]

        v = F.pad(v, (0, 0, 0, 1), mode="constant", value=1)    # [1, 32, 33, 496]
        vk = torch.matmul(v, trans_k)       # [1, 32, 33, 32]
        out = torch.matmul(vk, q)       # # [1, 32, 33, 496]
        if out.dtype == torch.bfloat16:
            out = out.float()
        out = out[:, :, :-1] / (out[:, :, -1:] + self.eps)  # [1, 32, 32, 496]

        out = torch.reshape(out, (B, N, C))     # [1, 1024, 16, 31]
        return out
    ...
    def forward(self, x):
        ...
        out = self.relu_linear_att(q,k,v,C)
